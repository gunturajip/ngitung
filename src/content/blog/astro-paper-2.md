...
All the blog studies were moved from `src/contents` to `src/content/blog` directory.
...
- const studyImportResult = import.meta.glob<MarkdownInstance<Frontmatter>>(
  "../contents/**/**/*.md",);

// new content fetching method
+ const studyImportResult = await getCollection("blog");
```

### Modified Search Logic for better Search Result

In the older version of AstroPaper, when someone search some article, the search criteria keys that will be searched are `title`, `description` and `headings` (heading means all the headings h1 ~ h6 of the blog study). In AstroPaper v2, only `title` and `description` will be searched as the user types.
...
### Default Tag for blog study

If a blog study doesn't have any tag (in other words, frontmatter property `tags` is not specified), the default tag `others` will be used for that blog study. But you can set the default tag in the `/src/content/_schemas.ts` file.
...
AstroPaper v2 has a new dark color scheme (high contrast & low contrast) which is based on Astro's dark logo. Check out [this link](https://astro-paper.pages.dev/studies/predefined-color-schemes#astro-dark) for more info.
...
All the [#docs](https://astro-paper.pages.dev/tags/docs/) blog studies and [README](https://github.com/satnaing/astro-paper#readme) are updated for this AstroPaper v2.

## Bug Fixes

- fix broken tags in the Blog study page
- in a tag page, the last part of the breadcrumb is now updated to lower-case for consistency
- exclude draft studies in a tag page
